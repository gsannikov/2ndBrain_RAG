import os, io, base64, requests, hashlib
from pathlib import Path
from typing import List
from PIL import Image

ENGINE = os.getenv("OCR_ENGINE","surya").lower()
CACHE_DIR = Path(os.getenv("OCR_CACHE_DIR","state/ocr_cache")); CACHE_DIR.mkdir(parents=True, exist_ok=True)

def _img_sha(img: Image.Image) -> str:
    b = io.BytesIO(); img.save(b, format="PNG"); data = b.getvalue()
...
def ocr_surya(images: List[Image.Image]) -> str:
    ...
    texts=[]
    for im in images:
        h=_img_sha(im); hit=_cache_get(h)
        if hit is not None: texts.append(hit); continue
        out = run_ocr([np.array(im)])
        txt = "\n".join(block["text"] for page in out for block in page["text_blocks"])
        _cache_put(h, txt); texts.append(txt)
    return "\n\f\n".join(texts)
...
def ocr_deepseek(images: List[Image.Image]) -> str:
    url = os.getenv("DEEPSEEK_OCR_URL"); model = os.getenv("DEEPSEEK_OCR_MODEL")
    texts=[]
    for im in images:
        h=_img_sha(im); hit=_cache_get(h)
        if hit is not None: texts.append(hit); continue
...
def run_ocr(images: List[Image.Image]) -> str:
    if ENGINE == "surya": return ocr_surya(images)
    if ENGINE == "paddle": return ocr_paddle(images)
    if ENGINE == "deepseek": return ocr_deepseek(images)
    return ""
