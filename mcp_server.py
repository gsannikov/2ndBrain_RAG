...
STATE_PATH = Path("state/ingest_state.json"); STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
...
def upsert_file(p: Path, state: dict):
    ...
    if p.suffix.lower() not in ALLOWED: return
    if p.stat().st_size > int(os.getenv("MAX_FILE_MB","80")) * (1<<20): return
...
def rag_get(path: str, start: int | None = None, end: int | None = None, window: int = 1200) -> TextContent:
    p = Path(path)
    if not p.exists(): return TextContent(text=json.dumps({"error":"not found"}))
    try:
        txt = read_text(p)
    except Exception as e:
        return TextContent(text=json.dumps({"error": str(e)}))
    if start is None and end is None:
        out = txt[:min(len(txt), window)]; span = [0, len(out)]
    else:
        s = max(0, (start or 0) - window//4)
        e = min(len(txt), (end or (start or 0)) + window//4)
        out = txt[s:e]; span = [s,e]
...
def rag_invalidate(path: str) -> TextContent:
    p = Path(path)
    COLL.delete(where={"path": str(p)})
    st = load_state(); st.pop(str(p), None); save_state(st)
